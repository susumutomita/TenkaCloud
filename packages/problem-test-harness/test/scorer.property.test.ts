...
const urlArbitrary = tokenArbitrary.map(
	(token) => `https://${token}.example.test/`,
);
...
const multiFlagInputArbitrary = multiFlagFixtureArbitrary.map(
	({ scoring, outputs }) => makeScoreInput(scoring, outputs),
);
...
const uptimeProbeCaseArbitrary: fc.Arbitrary<UptimeProbeCase> = fc
	...
	.map(([kind, outputKey, url, declaredStatuses, status, reachable]) => {
		const effectiveExpectedStatuses =
			declaredStatuses && declaredStatuses.length > 0
				? declaredStatuses
				: [200];
			...
			endpoints: [
				{
					slot: outputKey,
					path: "/",
					...(declaredStatuses === undefined
						? {}
						: { expectStatus: declaredStatuses }),
				},
			],
			pointsPerSuccess: 10,
		} as unknown as ProblemScoringMetadata;
		return {
			input: makeScoreInput(
				scoring,
				{ [outputKey]: url },
...
describe("runScorer property invariants", () => {
	...
	it("should succeed for multi-flag if and only if every declared output is non-empty", () => {
		fc.assert(
			fc.property(
				multiFlagFixtureArbitrary,
				({ scoring, outputs, outputValues }) => {
					const result = runScorer(makeScoreInput(scoring, outputs));
					const everyDeclaredOutputIsNonEmpty = outputValues.every(
						(value) => typeof value === "string" && value.length > 0,
					);
					expect(result.outcome === "success").toBe(
						everyDeclaredOutputIsNonEmpty,
					);
				},
			),
			PROPERTY_PARAMETERS,
