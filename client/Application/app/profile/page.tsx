...
export default function ProfilePage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    setEditError(null);
    try {
      const updated = await updateProfile({ name: trimmedName });
      setProfile(updated);
      setEditing(false);
    } catch (e) {
      setEditError(e instanceof Error ? e.message : t('profile.saveError'));
    } finally {
      setSaving(false);
    }
  }, [editName, t]);
            ...
            <CloudscapeContainer
              header={
                <CloudscapeHeader
                  ...
                  actions={
                    !editing ? (
                      <CloudscapeButton onClick={handleEdit}>
                        {t('profile.edit')}
                      </CloudscapeButton>
                    ) : undefined
                  }
                >
                  {editing ? t('profile.edit') : (profile?.name ?? 'ユーザー')}
                </CloudscapeHeader>
              }
            >
              {editing ? (
                <CloudscapeSpaceBetween size="m">
                  <CloudscapeFormField
                  ...
                  >
                    <CloudscapeInput
                      value={editName}
                      onChange={(e) => setEditName(e.detail.value)}
                    />
                  </CloudscapeFormField>
                  <CloudscapeSpaceBetween direction="horizontal" size="xs">
                    ...
                    <CloudscapeButton onClick={handleCancel}>
                      {t('profile.cancel')}
                    </CloudscapeButton>
