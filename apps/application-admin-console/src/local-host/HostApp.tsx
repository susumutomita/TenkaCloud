...
function HostLogin({ config }: { config: AppConfig }) {
  ...
  async function login(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok || !value.idToken || !value.accessToken || !value.refreshToken || typeof value.expiresAt !== "number") throw new Error(value.message ?? "主催者キーを確認してください。");
      auth.setTokens({
  ...
  return <div style={{ maxWidth: 560, margin: "64px auto" }}><Container header={<Header variant="h1">ローカル大会の主催者ログイン</Header>}>
    <form onSubmit={event => {
      event.preventDefault();
      void login();
    }}><SpaceBetween size="l">
        <p>起動したターミナルに表示された主催者キーを入力してください。参加者には、イベント作成後に発行するチームキーを配布します。</p>
        {error && <Alert type="error">{error}</Alert>}
        <FormField label="主催者キー"><Input
          type="password"
          value={key}
          onChange={({ detail }) => setKey(detail.value)}
          autoComplete="off" /></FormField>
        <Button variant="primary" formAction="submit" loading={busy} disabled={!key}>ログイン</Button>
      </SpaceBetween></form>
  </Container></div>;
}
  ...
  return <main style={{
    ...
    padding: "24px 32px"
      ...
      <Header
        ...
        actions={<SpaceBetween direction="horizontal" size="s"><Button onClick={() => navigate("/events")}>イベント一覧</Button><Button onClick={auth.logout}>ログアウト</Button></SpaceBetween>}>TenkaCloud / ローカル大会</Header>
      <Routes>
