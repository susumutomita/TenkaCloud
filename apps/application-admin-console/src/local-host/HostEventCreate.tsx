...
export function HostEventCreate({ config }: { config: AppConfig }) {
  ...
  const valid =
    ...
    slugs.every((value) => /^[a-z0-9](?:[a-z0-9-]{0,38}[a-z0-9])?$/u.test(value)) &&
    !!selected;
  ...
  return (
    <Container header={<Header variant="h2">イベントを作成</Header>}>
      <SpaceBetween size="l">
        ...
        <FormField
          label="チーム（1行に1チーム）"
          description="半角小文字・数字・ハイフンで指定。参加キーはチームごとに発行されます。"
        >
          <Textarea value={teams} onChange={({ detail }) => setTeams(detail.value)} rows={5} />
        </FormField>
        <FormField label="問題">
          <Select
        ...
        <Alert type="info">
          本体の起動・イベント作成にはDockerは不要です。この問題の環境準備にはDocker
          Composeを使い、チームごとに独立した環境を作成します。
        </Alert>
        <Button
          variant="primary"
          loading={busy}
