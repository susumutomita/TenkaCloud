/**
 * Tenant Admin (Application Plane) sign-in shell — design import "Application Plane Login.html".
 *
 * Presentation only: renders the shared `ConsoleAuthShell` (ink operator stage + form
 * panel + JA/EN toggle, web-kit) with `plane="app"`. The auth logic (Cognito Hosted UI
 * redirect, SAML routing, return-path) stays in `Login.tsx`; this shell only renders the
 * step area passed by it via these props.
 */
...
/** Shared Application-plane copy for the ConsoleAuthShell (title/subtitle come from props). */
export function applicationConsoleCopy(
  t: Translate,
  title: string,
  subtitle: string,
): ConsoleAuthCopy {
...
export interface ProductLoginShellProps {
  /** ページ見出し。 e.g. "TenkaCloud Application Admin Console" */
