...
bun --cwd infrastructure run pack install git \
  https://github.com/example/acme-pack.git \
...
```bash
bun --cwd infrastructure run pack install ./path/to/pack \
  --store ./.tenkacloud/pack-store
```
...
```bash
bun --cwd infrastructure run pack activate com.example.event-pack@1.0.0 \
  --tenant acme \
  --store ./.tenkacloud/pack-store
```
