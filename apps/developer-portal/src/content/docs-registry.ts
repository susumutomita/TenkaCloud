...
export const DOC_PAGES: readonly DocPage[] = [
  ...
  },
  {
    ...
    description: "Responsibilities, cloud and Docker components, and operation sequences.",
    maturity: "stable",
    section: "Concepts",
    headings: [
      { id: "reading-order", text: "Reading order" },
      { id: "cloud-responsibilities", text: "Cloud responsibilities" },
      { id: "cloud-components", text: "Cloud components" },
      { id: "local-play-and-docker", text: "Local play and Docker" },
      { id: "saas-tenant-onboarding", text: "SaaS tenant onboarding" },
      { id: "problem-deployment", text: "Problem deployment" },
      { id: "cloud-answers-and-scoring", text: "Cloud answers and scoring" },
      { id: "local-play-startup-and-scoring", text: "Local play startup and scoring" },
      { id: "next-steps", text: "Next steps" },
    ],
    body: "Platform architecture. Read the system context, use cases, logical responsibilities, components, operation sequences, then the physical Draw.io diagrams. SaaS uses SBT tenant onboarding and pooled or silo hosting; Lite omits SBT organization management. API authentication and scope checks enforce tenant boundaries alongside infrastructure. Cloud components separate hosting APIs, Step Functions problem deployment, participant APIs, coordination dispatch, scheduled scoring, and DynamoDB or supported Turso repositories. Cross-account deployment and status polling require STS AssumeRole with the mandatory ExternalId from SSM. Local play uses make local, SQLite, and sibling Docker problem containers; make local-dev supports opt-in Simulator. The Docker socket read-only mount does not restrict Docker API writes. Problem verifiers check answers; the platform updates scores and persists progress before responding. Cloud coordination state persistence and pending score delivery are separate. アーキテクチャ、プレーンの責務、システムコンテキスト、ユースケース、論理構成、コンポーネント、シーケンス、物理構成。テナント登録、問題配置、回答、採点、Docker、SQLite、Turso、データ保存。実装の構造と実 AWS の稼働確認・請求確認は区別する。",
  },
  {
