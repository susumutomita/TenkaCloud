...
export interface MachineApiOperation {
  ...
  readonly responses: Record<
    string,
    { readonly description: string } & { readonly [key: string]: unknown }
  >;
  readonly [key: string]: unknown;
}

export const MACHINE_API_SPEC: MachineApiSpec = {
  openapi: "3.1.0",
  info: {
    title: "TenkaCloud Machine API",
    version: "1.1.0",
    description:
      "Machine-to-machine surface of the TenkaCloud Tenant API (ADR-0005 Phase 1). A machine principal holds the role `TenantMachine`, which no destructive route accepts, so the operations below are the complete surface reachable with a machine credential. This reference is generated from the platform's own route table and validation schemas.",
    license: {
      name: "Apache-2.0",
  ...
  paths: {
    "/deployments": {
      get: {
        ...
        responses: {
          ...
          "401": {
            description: "The token is missing, expired, or lacks the required scope.",
          },
          "403": {
            description:
        ...
        security: [
          {
            TenkaCloudMachineOAuth: ["tenkacloud/ops.read"],
          },
        ],
        "x-tenkacloud-capability": "read",
      },
    },
    "/deployments/{jobId}": {
      get: {
        operationId: "getDeploymentsByJobId",
        ...
        parameters: [
          {
            ...
            schema: {
              type: "string",
              description: "Deployment job id (ULID).",
            },
          },
        ],
        responses: {
    ...
    "/deployments/{jobId}/stack-progress": {
      get: {
        operationId: "getDeploymentsByJobIdStackProgress",
    ...
    "/problems/{problemId}/deployments": {
      get: {
        operationId: "getProblemsByProblemIdDeployments",
    ...
    "/problems/{problemId}/deploy": {
      post: {
        operationId: "postProblemsByProblemIdDeploy",
        ...
        security: [
          {
            TenkaCloudMachineOAuth: ["tenkacloud/ops.deploy"],
          },
        ],
        "x-tenkacloud-capability": "deploy",
      },
    },
    "/deployments/retry": {
      post: {
        operationId: "postDeploymentsRetry",
        ...
        security: [
          {
            TenkaCloudMachineOAuth: ["tenkacloud/ops.write"],
          },
        ],
        "x-tenkacloud-capability": "write",
      },
    },
  },
  components: {
    securitySchemes: {
      TenkaCloudMachineOAuth: {
        type: "oauth2",
        description:
          "Cognito client credentials. The access token lives for 15 minutes. Every token must also carry the tenant binding scope `tc-tenant-<tenantId>/bind`; a token without exactly one binding scope is rejected.",
        flows: {
          clientCredentials: {
            tokenUrl: "{cognitoDomain}/oauth2/token",
            scopes: {
              "tenkacloud/ops.read": "Read deployments and events.",
              "tenkacloud/ops.deploy": "Start a deployment.",
              "tenkacloud/ops.write": "Re-queue failed deployments.",
            },
          },
        },
      },
