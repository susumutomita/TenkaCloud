...
# ===== Always-On runtime deployment role | Always-On runtimeデプロイrole =====
.PHONY: deploy-always-on-runtime-role synth-always-on-runtime-role destroy-always-on-runtime-role

# GitHub Actions が event 別 runtime を deploy/destroy するときに assume する OIDC role。
# event 非依存の one-time bootstrap (bin/tenkacloud-always-on-oidc.ts)。
# 必須 env: CDK_PARAM_AWS_ACCOUNT_ID / CDK_PARAM_AWS_REGION。
# 既存 GitHub OIDC provider を使う場合は CDK_PARAM_GITHUB_OIDC_PROVIDER_ARN も渡す。
ALWAYS_ON_RUNTIME_ROLE_APP := bunx tsx bin/tenkacloud-always-on-oidc.ts

deploy-always-on-runtime-role: ## Deploy the GitHub OIDC role for Always-On runtime workflows | Always-On runtime workflow用GitHub OIDC roleをdeploy
	@if [ -z "$${CDK_PARAM_AWS_ACCOUNT_ID}" ] || [ -z "$${CDK_PARAM_AWS_REGION}" ]; then \
	  echo "error: CDK_PARAM_AWS_ACCOUNT_ID / CDK_PARAM_AWS_REGION が未指定。" >&2; \
	  exit 1; \
	fi
	$(REPO_CDK) deploy --app "$(ALWAYS_ON_RUNTIME_ROLE_APP)" --all $(APPROVAL)

synth-always-on-runtime-role: export CDK_SKIP_BUNDLING := 1
synth-always-on-runtime-role: ## Synthesize the GitHub OIDC role for Always-On runtime workflows | Always-On runtime workflow用GitHub OIDC roleをsynth
	$(REPO_CDK) synth --app "$(ALWAYS_ON_RUNTIME_ROLE_APP)" --quiet

destroy-always-on-runtime-role: ## Delete the GitHub OIDC role for Always-On runtime workflows | Always-On runtime workflow用GitHub OIDC roleを削除
	@if [ -z "$${CDK_PARAM_AWS_ACCOUNT_ID}" ] || [ -z "$${CDK_PARAM_AWS_REGION}" ]; then \
	  echo "error: CDK_PARAM_AWS_ACCOUNT_ID / CDK_PARAM_AWS_REGION が未指定 (destroy も app synth のため必須)。" >&2; \
	  exit 1; \
	fi
	$(REPO_CDK) destroy --app "$(ALWAYS_ON_RUNTIME_ROLE_APP)" --all --force

# ===== Always-On OIDC command seam | Always-On OIDCコマンド境界 =====
.PHONY: deploy-always-on-command synth-always-on-command destroy-always-on-command

# Worker の OIDC IdP を IAM OIDC provider として登録し、frozen `tenkacloud.deploy` を
...
# 必須 env: CDK_PARAM_ALWAYS_ON_ISSUER_URL (= Worker origin) / CDK_PARAM_EVENT_BUS_ARN。
# 任意 env: CDK_PARAM_ALWAYS_ON_OIDC_PROVIDER_ARN / _COMMAND_SUBJECT / _COMMAND_ROLE_NAME。
...
deploy-always-on-command: ## Deploy the Always-On OIDC command seam | Always-On OIDC command seamをdeploy
	...
	$(REPO_CDK) deploy --app "$(ALWAYS_ON_COMMAND_APP)" --all $(APPROVAL)
...
synth-always-on-command: ## Synthesize the Always-On OIDC command seam | Always-On OIDC command seamをsynth
	$(REPO_CDK) synth --app "$(ALWAYS_ON_COMMAND_APP)" --quiet
...
destroy-always-on-command: ## Delete the Always-On OIDC command seam | Always-On OIDC command seamを削除
	...
	$(REPO_CDK) destroy --app "$(ALWAYS_ON_COMMAND_APP)" --all --force

# ===== Always-On per-event runtime | Always-Onイベント別runtime =====
.PHONY: deploy-always-on-runtime synth-always-on-runtime archive-always-on-runtime \
...
# 巻き込まない)。stack は runtime-tags を付与するので、手動 sweeper が期限切れ runtime を識別できる。
# 必須 env: lifecycle 3 値に加え、runtime scoring が参照する既存 event-data table 名、Workers URL、
# SSM SecureString の score-feed token parameter 名 (#2294)。
...
deploy-always-on-runtime: ## Deploy an event-scoped Always-On runtime | event単位のAlways-On runtimeをdeploy
	@if [ -z "$${CDK_PARAM_ALWAYS_ON_EVENT_ID}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_TENANT_ID}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_EXPIRES_AT}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_DEPLOYMENTS_TABLE_NAME}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_EVENTS_TABLE_NAME}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_ENDPOINTS_TABLE_NAME}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_CONTROL_PLANE_URL}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_RUNTIME_FEED_TOKEN_PARAMETER}" ] || [ -z "$${CDK_PARAM_ALWAYS_ON_ARCHIVE_BUCKET_NAME}" ]; then \
	  echo "error: Always-On runtime lifecycle/scoring の必須 CDK_PARAM が不足しています (#2294)。" >&2; \
	  exit 1; \
	fi
	$(REPO_CDK) deploy --app "$(ALWAYS_ON_RUNTIME_APP)" "$(ALWAYS_ON_RUNTIME_STACK)" $(APPROVAL)
...
synth-always-on-runtime: ## Synthesize an event-scoped Always-On runtime | event単位のAlways-On runtimeをsynth
	$(REPO_CDK) synth --app "$(ALWAYS_ON_RUNTIME_APP)" "$(ALWAYS_ON_RUNTIME_STACK)" --quiet
...
destroy-always-on-runtime: ## Delete an event-scoped Always-On runtime | event単位のAlways-On runtimeを削除
	...
	$(REPO_CDK) destroy --app "$(ALWAYS_ON_RUNTIME_APP)" "$(ALWAYS_ON_RUNTIME_STACK)" --force
