...
**2026-07-15 live-attempt blockers, now fixed in code.** The first live `CONTROL_DATA_BACKEND=turso` run got far enough to confirm the CFn-layer claim (`aws dynamodb list-tables` → `[]`; zero `AWS::DynamoDB::Table` in the deployed templates) before hitting three pure-SQL-only defects that synth/unit tests could not surface: the `DeployStatusWriter` OOM (`memorySize` raised to a live-safe value in #2656 and the root-cause CDK-in-bundle leak removed in #2657), the capacity panel returning 500 on zero event-hot tables (the DynamoDB capacity controls are now hidden entirely for SQL backends, #2658), and `teamLoginKey` being undistributable because the plaintext was discarded after creation (team login keys are now issued as one-time credentials shown once at creation and rotatable afterward, #2659). With those cleared, the remaining work is a fresh end-to-end live run of the runbook above — an operator step performed at `make deploy` time, not something CI or an offline check can execute.
