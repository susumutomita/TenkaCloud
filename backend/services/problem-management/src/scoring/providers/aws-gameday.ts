...
export class AWSGameDayScoringProvider implements IScoringExecutor {
  ...
  private async validateCriterion(
    ...
    // 検証タイプを決定
    const validationType = criterion.validationType || this.inferValidationType(criterion);
    ...
    try {
      return await validator(criterion, credentials, competitorAccountId, context);
    } catch (error) {
      return {
        passed: false,
        score: 0,
  ...
  private inferValidationType(criterion: ScoringCriterion): string {
    ...
    if (combined.includes('iam') || combined.includes('role') || combined.includes('policy')) {
      return 'iam-policy';
    }
    if (combined.includes('cloudwatch') || combined.includes('alarm') || combined.includes('monitoring')) {
      return 'cloudwatch-alarm';
    }
    if (combined.includes('security') || combined.includes('sg') || combined.includes('firewall')) {
  ...
  private registerDefaultValidators(): void {
    // EC2 インスタンス検証
    this.validators.set('ec2-instance', async (criterion, credentials, accountId, context) => {
      const instances = await this.describeEC2Instances(credentials, context.region, accountId);

      // 要件を解析
      const config = (criterion.validationConfig || {}) as EC2ValidationConfig;
      ...
      const matchingInstances = instances.filter(instance => {
        ...
        if (requiredType && instance.instanceType !== requiredType) return false;
        return true;
      });
    ...
    // S3 バケット検証
    this.validators.set('s3-bucket', async (criterion, credentials, _accountId, context) => {
      ...
      return {
        ...
        maxScore: criterion.maxPoints,
        feedback: checks.length > 0 ? checks.join(', ') : `✓ S3 バケット検証完了`,
      };
    });

    // Lambda 関数検証
    this.validators.set('lambda-function', async (criterion, credentials, _accountId, context) => {
      const functions = await this.listLambdaFunctions(credentials, context.region);

      const config = (criterion.validationConfig || {}) as LambdaValidationConfig;
      ...
      if (requiredRuntime && matchingFunction.runtime === requiredRuntime) {
        ...
        checks.push(`✗ 期待ランタイム: ${requiredRuntime} (実際: ${matchingFunction.runtime})`);
      }
    ...
    this.validators.set('cloudformation-stack', async (criterion, credentials, _accountId, context) => {
      const config = (criterion.validationConfig || {}) as CloudFormationValidationConfig;
      const stackName = config.stackName ?? `tenkacloud-${context.problemId}`;

      const stack = await this.describeStack(credentials, context.region, stackName);
      ...
      const isComplete = stack.status === 'CREATE_COMPLETE' || stack.status === 'UPDATE_COMPLETE';

      return {
        passed: isComplete,
        score: isComplete ? criterion.maxPoints : Math.round(criterion.maxPoints * 0.5),
        maxScore: criterion.maxPoints,
        feedback: isComplete
          ? `✓ スタック正常: ${stackName}`
    ...
    // VPC 設定検証
    this.validators.set('vpc-config', async (criterion, credentials, _accountId, context) => {
      const vpcs = await this.describeVPCs(credentials, context.region);

      const config = (criterion.validationConfig || {}) as VPCValidationConfig;
      ...
      if (vpcs.length > 0) {
        ...
        const totalSubnets = vpcs.reduce((sum, vpc) => sum + vpc.subnetCount, 0);
    ...
    // セキュリティグループ検証
    this.validators.set('security-group', async (criterion, credentials, _accountId, context) => {
      const securityGroups = await this.describeSecurityGroups(credentials, context.region);

      const config = (criterion.validationConfig || {}) as SecurityGroupValidationConfig;
      ...
      for (const sg of securityGroups) {
        for (const rule of sg.inboundRules) {
          if (rule.source === '0.0.0.0/0' && prohibitedPorts.includes(rule.port)) {
            hasViolation = true;
            score -= criterion.maxPoints * 0.3;
            checks.push(`✗ ${sg.name}: ポート ${rule.port} がパブリックに開放`);
          }
        }
      }
    ...
    // IAM ポリシー検証
    this.validators.set('iam-policy', async (criterion, credentials, _accountId, _context) => {
      const roles = await this.listIAMRoles(credentials);

      const config = (criterion.validationConfig || {}) as IAMValidationConfig;
    ...
    // CloudWatch アラーム検証
    this.validators.set('cloudwatch-alarm', async (criterion, credentials, _accountId, context) => {
      const alarms = await this.listCloudWatchAlarms(credentials, context.region);

      const config = (criterion.validationConfig || {}) as CloudWatchValidationConfig;
      ...
      if (requiredMetrics.length > 0) {
        ...
        const missingMetrics = requiredMetrics.filter((m: string) => !configuredMetrics.includes(m));
    ...
    // API Gateway 検証
    this.validators.set('api-gateway', async (criterion, credentials, _accountId, context) => {
      const apis = await this.listAPIGateways(credentials, context.region);

      const config = (criterion.validationConfig || {}) as APIGatewayValidationConfig;
    ...
    // 汎用検証（フォールバック）
    this.validators.set('generic', async (criterion, _credentials, _accountId, _context) => {
      // 汎用検証 - 常に部分点を付与
      return {
        passed: true,
  ...
  private async listS3Buckets(
    ...
    return [
      { name: 'mock-bucket', versioning: true, encryption: true },
    ];
  }
  ...
  private async listLambdaFunctions(
    ...
    return [
      { name: 'mock-function', runtime: 'nodejs20.x' },
    ];
  }
  ...
  private async describeVPCs(
    ...
    return [
      { vpcId: 'vpc-mock', subnetCount: 4, azCount: 2 },
    ];
  }
  ...
  private async describeSecurityGroups(
    _credentials: CloudCredentials,
    _region: string
  ): Promise<{ name: string; inboundRules: { port: number; source: string }[] }[]> {
  ...
  private async listIAMRoles(
    ...
    return [
      { name: 'mock-role', hasAdminAccess: false },
    ];
  }
  ...
  private async listCloudWatchAlarms(
    ...
    return [
      { name: 'mock-alarm', metric: 'CPUUtilization' },
    ];
  }
