...
describe("makeCoordinationScopeResolver", () => {
  ...
  it.each([
    "TEARDOWN",
    "ARCHIVED",
  ])("should refuse write scope when the owning event is %s", async (eventStatus) => {
    const send = vi.fn(async (command: unknown) => {
      if (command instanceof GetCommand) {
        return { Item: { tenantId: "tn1", eventId: "e1", status: eventStatus } };
      }
      return {
        Items: [
          { tenantId: "tn1", eventId: "e1", teamId: "t1", problemId: "p1", status: "FAILED" },
        ],
      };
    });
    const resolve = makeCoordinationScopeResolver(fakeParticipantShared(send), config);

    expect(await resolve("key", "write")).toBeNull();
    // Read-only debrief/projection remains available during teardown.
    expect(await resolve("key", "read")).not.toBeNull();
  });
