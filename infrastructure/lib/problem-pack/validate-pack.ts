/**
 * [Problem Packs / Issue #2088] Standalone, offline pack validator.
 *
 ...
 * credentials, no network, and no core-repo edits. It performs only read-only
 * filesystem I/O inside the pack root.
 *
 * Responsibilities (the manifest parser of #2087 is intentionally I/O-free, so
 * these all live here):
 *   - read + schema-validate `<dir>/tenkacloud-pack.json` (via `parsePackManifest`)
 *   - safe path resolution: every resolved path (problemsRoot, discovered
 *     metadata, referenced artifacts) must stay strictly inside the pack root —
 *     `..` traversal, absolute paths, and escaping symlinks are rejected
 *   - discover `<problemsRoot>/<category>/<id>/metadata.json`
 *   - reuse the existing metadata / runtime / scoring / endpoint / phase /
 *     disruption / coordination validators
