/**
 * [Problem Packs / Issue #2088] Safe path resolution + read-only filesystem
 * helpers for the offline pack validator.
 *
 * Split out of `validate-pack.ts` (SRP): this module owns the security-critical
 * boundary check — "does this author-supplied path stay strictly inside the pack
 * root?" — independently of the validation orchestration. It rejects absolute
 * paths, `..` traversal, and symlinks that escape the boundary (resolved on the
 * longest existing prefix so a not-yet-created artifact is still checked).
 *
