...
/**
 * Issue #1242 / #2211: route 入口で JSON body / query / path param を zod schema で検証する
 * 共通ヘルパー。実装は `handlers/shared/http-parse.ts` に集約され、participant / event /
 * deploy が同一実装を共有する (`invalid_body` / `validation_failed` の JSON 形状はフロント
 * との契約なので 1 箇所で管理する)。ここは既存 import 経路を保つための re-export。
 */
