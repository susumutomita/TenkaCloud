 ...
 * {@link SqlDeploymentsCore} engine, and every port method below delegates to
...
export class SqlDeploymentsRepository implements DeploymentsRepository {
  private readonly query: SqlDeploymentsQuery;
  private readonly lifecycle: SqlDeploymentsLifecycle;
  private readonly scoring: SqlDeploymentsScoring;
  private readonly composite: SqlDeploymentsComposite;
  private readonly coordination: SqlDeploymentsCoordination;

  constructor(sql: SqlExecutor) {
    const core = new SqlDeploymentsCore(sql);
    this.query = new SqlDeploymentsQuery(core);
    this.lifecycle = new SqlDeploymentsLifecycle(core);
    this.scoring = new SqlDeploymentsScoring(core);
    this.composite = new SqlDeploymentsComposite(core);
    this.coordination = new SqlDeploymentsCoordination(core);
  }
