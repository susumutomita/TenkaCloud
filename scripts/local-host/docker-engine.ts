...
import { composeArgsForCli, generateSecretEnv, isComposeUnitRunning, resolveComposeCli } from "../local-play/docker-adapter";
import { loadContainerProblem, type ContainerProblem } from "../local-play/manifest";
import { remapComposeHostPorts, remapContainerProblem } from "../local-play/port-remap";
import { parseLocalPlaySnapshot, restoreLocalPlayState, snapshotLocalPlayState } from "../local-play/state-store";
...
interface Definition { problem: ContainerProblem; hashes: Record<string, string>; composeText: string }
...
function loadCatalog(repositoryRoot: string): Problem[] {
  ...
  if (problem.scoring.kind !== "verify" || problem.terminal || Object.keys(problem.challengeEndpoints).join() !== "Web") {
...
function definitionOf(job: Job, verifySources: boolean): Definition {
  ...
  for (const [path, expected] of Object.entries(definition.hashes)) {
    if (fingerprint(join(definition.problem.problemDir, path)) !== expected) {
      throw new Error("The event's pinned problem files changed. Restore the original catalog before continuing.");
    }
  }
  return definition;
}

async function compose(unit: LocalComposeUnit, action: "up" | "down", environment: NodeJS.ProcessEnv): Promise<void> {
  const cli = resolveComposeCli();
  const args = composeArgsForCli(cli, unit.composePath, unit.composeProjectName, action, unit.projectDirectory);
  await new Promise<void>((accept, reject) => {
    const child = spawn(cli.command, args, { env: environment, stdio: ["ignore", "ignore", "pipe"] });
    let tail = "";
    ...
    child.once(
      ...
      code => {
        ...
        if (code === 0) accept();
          ...
          reject(new Error(daemon ? "Docker daemon is unavailable. Start Docker Desktop or Engine and retry." : `Docker Compose ${action} failed (exit ${String(code)}). Inspect project ${unit.composeProjectName} on the host.`));
        }
      }
    );
  });
...
export class DockerHostingEngine implements RuntimeEngine {
  ...
  constructor(repositoryRoot: string, private readonly dataDirectory: string) {
    this.problems = loadCatalog(repositoryRoot);
  }
  catalog(): readonly Problem[] {
    return this.problems;
  }
  private plan(job: Job, verifySources = true): {
    ...
    assertComposePolicy(source, { problemDir: problem.problemDir, composePath: problem.composePath });
  ...
  async start(job: Job, retain: (unit: string | null) => void): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Problem startup failed and cleanup was incomplete. Runtime ownership was retained; use host teardown to retry.", { cause: error });
      }
      throw error;
    }
  }
  async recover(job: Job): Promise<void> {
    ...
    if (readFileSync(unit.composePath, "utf8") !== plan.composeText) throw new Error("Recorded runtime composition changed; refusing to adopt it.");
    if (!isComposeUnitRunning(unit)) throw new Error("The recorded problem environment is not running. Retry deployment from the host console.");
    await ready(plan.started.problem.verifyUrl);
    await ready(this.surfaceFrom(plan.started));
    this.running.set(job.jobId, plan.started);
  }
  private validatedUnit(job: Job, expected: LocalComposeUnit): LocalComposeUnit {
    ...
    for (const key of [
      ...
      if (unit[key] !== expected[key]) throw new Error("Runtime ownership does not match this team/problem.");
    }
    if (JSON.stringify(unit.secretEnv) !== JSON.stringify(expected.secretEnv)) throw new Error("Runtime secret declaration changed.");
    return unit;
  }
  async stop(job: Job): Promise<void> {
    ...
    if (!existsSync(unit.composePath)) writeFileSync(unit.composePath, plan.composeText, { flag: "wx", mode: 0o600 });
    if (readFileSync(unit.composePath, "utf8") !== plan.composeText) throw new Error("Recorded runtime composition changed; refusing unsafe cleanup.");
    ...
    const cleanupEnvironment = Object.fromEntries(unit.secretEnv.map(name => [name, "tenkacloud-host-cleanup"]));
  ...
  private async state(context: Context) {
    const problems = context.event.problems.map(problem => (JSON.parse(problem.definition) as Definition).problem);
    const state = createLocalPlayState(
      { problems },
      {
        teamName: context.team.displayName,
        maxRunning: Math.max(1, problems.length),
        verify: (url, submission, verifyContext, options) => verifySubmission(
          url,
          submission,
          { ...verifyContext, teamId: context.team.teamId },
          {
            ...options,
            fetchImpl: (input, init) => fetch(input, { ...init, signal: AbortSignal.timeout(5000) }),
          }
        ),
        startContainer: async problem => {
          const job = context.jobs.find(candidate => candidate.problemId === problem.problemId && candidate.status === "COMPLETE");
    ...
    if (context.team.snapshot) restoreLocalPlayState(state, parseLocalPlaySnapshot(context.team.snapshot));
    for (const job of context.jobs) {
      if (job.status === "COMPLETE" && this.running.has(job.jobId)) await state.lifecycle.ensureRunning(job.problemId);
    }
    return state;
  }
  ...
  private result(
    ...
    const completedProblems = [...state.runtimes.values()].filter(runtime => runtime.problem.scoring.kind === "verify"
      ? runtime.solved.has(runtime.problem.problemId)
      : runtime.problem.scoring.checks.every(check => runtime.solved.has(check.id))).length;
    return {
      ...
      completedProblems,
      scoreEvents: state.scoreEvents.map(event => ({ ...event, jobId: jobs.get(event.problemId) ?? event.jobId }))
    };
  }
