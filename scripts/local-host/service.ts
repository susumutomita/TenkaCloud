import { id, issueSession, secret, SerialQueue } from "./auth";
import {
  assertPlaying,
  gate,
  HostError,
...
export class HostingService {
  ...
  private detail(event: HostedEvent, query: URLSearchParams) {
    ...
    return {
      ...
      problems: event.problems.map(problem => ({ problemId: problem.problemId, defaultRegion: "local" })),
      deploymentsByProblem: Object.fromEntries(event.problems.map(problem => [
        problem.problemId,
        jobs.filter(job => job.problemId === problem.problemId).map(job => ({
          jobId: job.jobId,
          teamId: job.teamId,
          status: job.status,
  ...
  async admin(request: ApiRequest): Promise<ApiResponse> {
    ...
    if (parts[0] !== "events" || !parts[1] || !eventPattern.test(parts[1])) throw new HostError(404, "Unknown host endpoint.");
    const eventId = parts[1];
    if (parts.length === 2 && request.method === "GET") return ok(this.detail(this.currentEvent(eventId), request.query));
    return this.queue.run(eventId, async () => this.mutateEvent(this.currentEvent(eventId), parts.slice(2), request));
  }
  private createEvent(body: Record<string, unknown>): ApiResponse {
    const name = text(body.name, "name");
    if (!Array.isArray(body.teams) || body.teams.length < 1 || body.teams.length > MAX_JOBS) throw new HostError(400, "Choose 1–40 teams.");
    if (!Array.isArray(body.problems) || body.problems.length < 1 || body.problems.length > MAX_JOBS) throw new HostError(400, "Choose at least one supported problem.");
    ...
    const problems = body.problems.map(value => {
      ...
      if (!problem) throw new HostError(422, `Problem is not supported by local hosting: ${problemId}`);
      return { ...problem };
    });
    if (new Set(problems.map(problem => problem.problemId)).size !== problems.length) throw new HostError(400, "Duplicate problem.");
    if (body.teams.length * problems.length > MAX_JOBS) throw new HostError(422, "Local hosting supports at most 40 simultaneous team/problem environments.");
    ...
    const teams: Team[] = body.teams.map(value => {
      ...
      if (!slugPattern.test(internalSlug)) throw new HostError(400, "Use lowercase letters, digits and hyphens for team slugs.");
      return {
        teamId: id(timestamp),
        eventId,
        internalSlug,
    ...
    if (new Set(teams.map(team => team.internalSlug)).size !== teams.length) throw new HostError(400, "Duplicate team slug.");
    ...
    return ok(
      {
        ...this.summary(event),
        problems: problems.map(problem => ({ problemId: problem.problemId, defaultRegion: "local" })),
        teams: teams.map(team => ({
          teamId: team.teamId,
          internalSlug: team.internalSlug,
  ...
  private async mutateEvent(event: HostedEvent, parts: string[], request: ApiRequest): Promise<ApiResponse> {
    const command = parts.join("/");
    if (this.busyEvents.has(event.eventId)) throw new HostError(409, "An environment operation is already in progress.");
    if (request.method === "POST" && command === "deploy") return this.deploy(event, object(request.body));
    if (request.method === "DELETE" && command === "") return this.teardown(event);
    if (request.method === "PATCH" && command === "schedule") return this.schedule(event, object(request.body));
    if (request.method === "POST" && command === "end") {
      ...
      return ok({ endsAt: event.endsAt, updatedDeployments: this.store.jobs(event.eventId).length });
    }
    if (command === "lock-scoring" && ["POST", "DELETE"].includes(request.method)) {
      if (!["READY", "ENDED"].includes(event.status)) throw new HostError(409, "Event is not lockable.");
    ...
    if (command === "archive" && request.method === "POST") {
      if (!["DRAFT", "ENDED", "TEARDOWN"].includes(event.status)) throw new HostError(409, "Event is not archivable.");
      if (this.store.jobs(event.eventId).some(job => job.unit)) throw new HostError(409, "Tear down all environments before archiving.");
    ...
    if (command === "notifications" && request.method === "POST") {
      ...
      if (severity !== "info" && severity !== "warning") throw new HostError(400, "Invalid notification severity.");
    ...
    if (parts.length === 3 && parts[0] === "teams" && parts[2] === "rotate-login-key" && request.method === "POST") {
  ...
  private schedule(event: HostedEvent, body: Record<string, unknown>): ApiResponse {
    if (event.status !== "READY") throw new HostError(409, "Deploy the event before scheduling it.");
    if (Object.keys(body).some(key => ![
      "startNow",
      "startsAt",
      "endsAt",
      "scoreboardFreezeMinutes"
    ].includes(key))) throw new HostError(422, "Automatic deploy/teardown schedules are not supported locally.");
    if (body.startNow !== undefined && body.startNow !== true) throw new HostError(400, "startNow must be true.");
    if (body.startNow && body.startsAt !== undefined) throw new HostError(400, "Choose startNow or startsAt, not both.");
    const parseTime = (value: unknown): string => {
      const input = text(value, "timestamp", 40);
      const milliseconds = Date.parse(input);
      if (!/^\d{4}-\d{2}-\d{2}T.*(?:Z|[+-]\d{2}:\d{2})$/u.test(input) || !Number.isFinite(milliseconds)) throw new HostError(400, "An ISO timestamp with a timezone is required.");
    ...
    if (event.startsAt && event.endsAt && Date.parse(event.endsAt) <= Date.parse(event.startsAt)) throw new HostError(400, "The end must be after the start.");
    if (body.scoreboardFreezeMinutes !== undefined) {
      const minutes = body.scoreboardFreezeMinutes;
      if (typeof minutes !== "number" || !Number.isInteger(minutes) || minutes < 0 || minutes > 180) throw new HostError(400, "Freeze minutes must be an integer from 0 to 180.");
  ...
  private deploy(event: HostedEvent, body: Record<string, unknown>): ApiResponse {
    if (!["DRAFT", "DEPLOYING"].includes(event.status)) throw new HostError(409, "This event has already been deployed.");
    if (Object.keys(body).some(key => key !== "retryFailedOnly") || (body.retryFailedOnly !== undefined && body.retryFailedOnly !== true)) throw new HostError(422, "Only full deployment or failed-environment retry is supported locally.");
    const teams = this.store.teams(event.eventId);
    const existing = this.store.jobs(event.eventId);
    const occupied = new Set(this.store.jobs().filter(job => job.status !== "DELETED").map(job => job.offset));
    const targets: Job[] = [];
    for (const team of teams) for (const problem of event.problems) {
      const previous = existing.find(job => job.teamId === team.teamId && job.problemId === problem.problemId);
      if (previous?.status === "COMPLETE") continue;
      if (previous) {
        targets.push(previous);
        continue;
      }
      const slot = Array.from({ length: MAX_JOBS }, (_, index) => (index + 1) * 1000).find(offset => !occupied.has(offset));
      if (slot === undefined) throw new HostError(422, "No free runtime slots. Tear down another event first.");
      occupied.add(slot);
      targets.push({
        jobId: id(this.now()),
    ...
    this.launch(
      ...
      async () => {
        ...
        if (this.store.jobs(event.eventId).every(job => job.status === "COMPLETE")) latest.status = "READY";
    ...
    return ok(
      {
        ...
        skipped: existing.length - targets.filter(job => existing.some(prior => prior.jobId === job.jobId)).length
      },
      202
    );
  }
      ...
      async () => {
        for (const job of jobs) {
          ...
          try {
            ...
            job.status = "FAILED";
            job.error = error instanceof Error ? error.message.slice(0, 2000) : "Cleanup failed; ownership retained.";
          }
          this.store.putJob(job);
        }
      }
  ...
  private launch(eventId: string, operation: () => Promise<void>): void {
    ...
    const task = Promise.resolve().then(operation).catch(error => {
      this.log(`Local-host operation failed for ${eventId}: ${error instanceof Error ? error.message : String(error)}`);
    }).finally(() => {
  ...
  async recover(): Promise<void> {
    for (const job of this.store.jobs()) {
      ...
      try {
        ...
        job.status = "FAILED";
        job.error = error instanceof Error ? error.message.slice(0, 2000) : "Runtime recovery failed.";
      }
      this.store.putJob(job);
    }
    for (const event of this.store.events()) {
      ...
      if (["DEPLOYING", "READY"].includes(event.status)) {
        const recovered = jobs.length === event.problems.length * this.store.teams(event.eventId).length && jobs.every(job => job.status === "COMPLETE");
  ...
  async participant(request: ApiRequest): Promise<ApiResponse> {
    ...
    const hintMatch = /^\/portal\/me\/problems\/([^/]+)\/hints\/([^/]+)\/reveal$/u.exec(request.path);
    if (request.path !== "/portal/me/submit-flag" && !hintMatch) throw new HostError(404, "Participants cannot deploy, reset or stop competition environments.");
    const body = hintMatch ? {} : object(request.body);
    if (!hintMatch && (typeof body.flag !== "string" || body.flag.length > 16_384 || typeof body.problemId !== "string")) throw new HostError(400, "Invalid flag submission.");
    if ("teamId" in body || "eventId" in body) throw new HostError(400, "Team and event identity come from authentication, not the request body.");
    return this.queue.run(
      team.eventId,
      async () => {
        ...
        if (request.nonce) {
          if (!/^[A-Za-z0-9_-]{8,128}$/u.test(request.nonce)) throw new HostError(400, "Invalid Idempotency-Key.");
        ...
        if (!current.jobs.some(job => job.problemId === problemId && job.status === "COMPLETE")) throw new HostError(409, "This team's problem environment is not running.");
        ...
        this.store.transaction(() => {
          ...
          if (request.nonce) this.store.putReceipt(fresh.teamId, request.nonce, fingerprint, result.status, result.body);
        });
  ...
  private async participantRead(context: Context, path: string): Promise<ApiResponse> {
    switch (path) {
      case "/portal/me": return ok(await this.teamView(context));
      case "/portal/me/score-events": return ok({ entries: context.team.scoreEvents.slice(0, 100) });
      case "/portal/me/notifications": return ok({ eventId: context.event.eventId, items: this.store.notifications(context.event.eventId) });
      case "/portal/me/deploy-logs": return ok({ entries: [], cursor: "" });
      case "/portal/leaderboard": return ok(this.leaderboard(context));
      case "/portal/leaderboard/score-events": {
        ...
        return ok({
          ...
          teams: this.store.teams(context.event.eventId).map(team => ({
            ...
            events: team.scoreEvents.filter(event => cutoff === null || Date.parse(event.occurredAt) < cutoff),
          }))
        });
      }
      default: throw new HostError(404, "Unknown participant endpoint.");
    }
  }
  private freezeCutoff(event: HostedEvent): number | null {
    if (!event.endsAt || event.scoreboardFreezeMinutes === 0 || event.status === "ENDED") return null;
  ...
  private leaderboard(context: Context) {
    const cutoff = this.freezeCutoff(context.event);
    const entries = this.store.teams(context.event.eventId).map(team => {
      const historical = team.scoreEvents.filter(event => cutoff !== null && Date.parse(event.occurredAt) < cutoff);
      return {
        teamId: team.teamId,
        teamName: team.displayName,
        score: cutoff === null ? team.score : historical.reduce((sum, event) => sum + event.points, 0),
        completedProblems: cutoff === null ? team.completedProblems : new Set(historical.filter(event => event.source === "flag" && event.result === "ok").map(event => event.problemId)).size,
  ...
  private async teamView(context: Context): Promise<Record<string, unknown>> {
    ...
    const safeProblems = await Promise.all(problems.map(async raw => {
      ...
      const notice = "> 競技モードでは、問題環境の起動・停止は主催者が行います。問題文の「起動」操作は不要です。競技中は「アクセス先 URL」の Web を開いてください。\n\n";
      if (typeof problem.instructions === "string") problem.instructions = notice + problem.instructions;
      if (problem.i18n) {
        const english = { ...object(object(problem.i18n).en) };
        if (typeof english.instructions === "string") {
          english.instructions = "> In competition mode, the organizer starts and stops your environment. Skip the problem's Start instruction and open Web under Access URLs during the event.\n\n" + english.instructions;
        }
      ...
      problem.stackOutputs = job?.status === "COMPLETE" && eventGate.kind === "ok" && this.surfaceLink
