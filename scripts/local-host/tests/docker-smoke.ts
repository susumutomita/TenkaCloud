...
async function main(): Promise<void> {
  ...
  async function attach(): Promise<void> {
    ...
    writeFileSync(join(staticRoot, "host.html"), "<!doctype html><title>Docker integration check</title>");
  ...
  async function api<Body>(role: "host" | "portal", path: string, method = "GET", body?: unknown, credential?: string): Promise<Body> {
    const endpoint = role === "host" ? host : portal;
    assert.ok(endpoint);
    const token = credential ?? (role === "host" ? accessToken : "");
    const response = await fetch(
      `${endpoint.origin}/api${path}`,
      {
        method,
        headers: {
  ...
  async function login(): Promise<void> {
    const session = await api<{ idToken: string }>("host", "/host/login", "POST", { key: masterKey }, "");
  ...
  async function solve(team: { teamLoginKey: string }): Promise<string> {
    ...
    const response = await fetch(
      `${origin}/login`,
      {
        ...
        body: new URLSearchParams({ username: "admin' --", password: "not-known" }),
      }
    );
  ...
  try {
    await attach();
    await login();
    const event = await api<CreatedEvent>(
      "host",
      "/events",
      "POST",
      {
    ...
    checks.push("Two production Docker environments, separate projects/ports and durable ownership");
    await api(
      "host",
      `/events/${eventId}/schedule`,
      "PATCH",
      {
        startNow: true, endsAt: new Date(Date.now() + 10 * 60_000).toISOString(),
      }
    );
    const flagA = await solve(first);
    const flagB = await solve(second);
    assert.notEqual(flagA, flagB);
    checks.push("Unmodified sqli-demo solved over isolated authorized HTTP gateways; per-team secrets differ");
    ...
    checks.push("Repository scorer/verifier reused; concurrent submissions award once and a foreign flag is rejected");
    await restart();
    assert.equal(store.team(first.teamId).score, 100);
    assert.equal(store.team(second.teamId).score, -5);
    assert.equal(await solve(first), flagA);
    assert.equal(await solve(second), flagB);
    checks.push("New production adapter instance recovers real running Docker projects and SQLite progress");
    ...
    checks.push("End state survives restart; physical teardown removes owned environments without deleting results");
    console.log(checks.map(check => `PASS ${check}`).join("\n"));
    if (process.env.HOST_DOCKER_REPORT) writeFileSync(
    ...
    if (cleanupErrors.length) {
      ...
      if (!failed) throw new AggregateError(cleanupErrors, "Production smoke cleanup failed.");
