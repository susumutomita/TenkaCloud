...
interface ResponseData<Body = Record<string, unknown>> { status: number; body: Body; response: Response }
...
async function fixture(operation: (fixture: Awaited<ReturnType<typeof createFixture>>) => Promise<void>): Promise<void> {
...
async function createFixture() {
  ...
  let service = new HostingService(store, engine, key, () => clock, () => { });
  ...
  async function begin(eventId: string, duration = 60_000): Promise<void> {
    const result = await request(
      ...
      "PATCH",
      { startNow: true, endsAt: new Date(clock + duration).toISOString() }
    );
    assert.equal(result.status, 200, JSON.stringify(result.body));
  }
  async function challenge(teamKey: string) {
    const view = await request<PortalView>("participant", "/api/portal/me", "GET", undefined, teamKey);
    ...
    const attacked = await fetch(
      `${origin}/login`,
      {
        ...
        body: new URLSearchParams({ username: "admin' --", password: "unknown" })
      }
    );
  ...
  async function restart(): Promise<void> {
    ...
    service = new HostingService(store, engine, key, () => clock, () => { });
...
test(
  "No automatic login, and public configuration has no host/team credentials",
  () => fixture(async f => {
...
test(
  "Host and participant listeners reject the other role",
  () => fixture(async f => {
    const event = await f.create();
    assert.equal(
      (await f.request("admin", "/api/events", "GET", undefined, required(event.teams[0]).teamLoginKey)).status,
      401
    );
    assert.equal(
    ...
    assert.equal((await f.request("participant", "/api/host/login", "POST", { key: f.key })).status, 404);
  })
);
test(
  "Host/Origin, malformed bodies and private filesystem routes fail closed",
  () => fixture(async f => {
    assert.equal(
      (await f.request("admin", "/api/events", "GET", undefined, undefined, { origin: "https://evil.example" })).status,
      403
    );
...
test(
  "Real HTTP exercise, separate team flags and single award under concurrent submissions",
  () => fixture(async f => {
    const event = await f.create();
...
test(
  "Idempotent wrong submissions, hint charges and payload conflict detection",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
    await f.begin(event.eventId);
    ...
    for (let index = 0; index < 2; index++) assert.equal((await f.request("participant", path, "POST", {}, a.teamLoginKey)).status, 200);
    assert.equal(f.store.team(a.teamId).score, -25);
  })
);
test(
  "Start, lock and end gates also prevent early instructions and writeup disclosure",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
    const a = required(event.teams[0]);
    const before = await f.request<PortalView>("participant", "/api/portal/me", "GET", undefined, a.teamLoginKey);
    ...
    await f.begin(event.eventId);
    const during = await f.request<PortalView>("participant", "/api/portal/me", "GET", undefined, a.teamLoginKey);
    ...
    assert.equal((await f.request("admin", `/api/events/${event.eventId}/end`, "POST", {})).status, 200);
    assert.equal((await submit()).body.kind, "scoring_ended");
  })
);
test(
  "Verifier completion after the server deadline does not persist a score",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
...
test(
  "Challenge handoffs are one-use, private, route-limited and revoked by team-key rotation",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
    await f.begin(event.eventId);
    ...
    await csrf.text();
    const rotated = await f.request<{ teamLoginKey: string }>("admin", `/api/events/${event.eventId}/teams/${a.teamId}/rotate-login-key`, "POST", {});
    ...
    await revoked.text();
    assert.equal(
      (await f.request("participant", "/api/portal/me", "GET", undefined, rotated.body.teamLoginKey)).status,
      200
    );
  })
);
test(
  "Restart preserves teams, credentials, score history, receipt and real runtime",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
    await f.begin(event.eventId);
    ...
    assert.equal(
      (await f.request("participant", "/api/portal/me/submit-flag", "POST", body, a.teamLoginKey, headers)).status,
      200
    );
    ...
    assert.equal(f.store.team(a.teamId).scoreEvents.length, 1);
    assert.equal(
      (await f.request("participant", "/api/portal/me/submit-flag", "POST", body, a.teamLoginKey, headers)).body.kind,
      "ok"
    );
    assert.equal(f.store.team(a.teamId).scoreEvents.length, 1);
...
test(
  "Runtime failures stay failures; retry and teardown preserve ownership on failure",
  () => fixture(async f => {
    const event = await f.create();
    f.engine.failStart = true;
    await f.deploy(event.eventId);
    assert.equal(f.store.event(event.eventId).status, "DEPLOYING");
    assert.ok(f.store.jobs(event.eventId).every(job => job.status === "FAILED" && job.unit !== null));
    f.engine.failStart = false;
    await f.deploy(event.eventId);
    await f.begin(event.eventId);
    f.engine.failStop = true;
    assert.equal((await f.request("admin", `/api/events/${event.eventId}`, "DELETE", {})).status, 202);
    await f.service.drain();
    assert.ok(f.store.jobs(event.eventId).every(job => job.status === "FAILED" && job.unit !== null));
    assert.equal((await f.request("admin", `/api/events/${event.eventId}/archive`, "POST", {})).status, 409);
    f.engine.failStop = false;
    await f.request("admin", `/api/events/${event.eventId}`, "DELETE", {});
    await f.service.drain();
    assert.ok(f.store.jobs(event.eventId).every(job => job.status === "DELETED" && job.unit === null));
    assert.equal(f.store.teams(event.eventId).length, 2);
  })
);
test(
  "Teardown never affects another event; participants cannot reset or stop",
  () => fixture(async f => {
    ...
    const foreign = await f.request("admin", `/api/events/${first.eventId}/teams/${b.teamId}/rotate-login-key`, "POST", {});
    assert.equal(foreign.status, 404);
  })
);
test(
  "Session revocation, expiration and invalid-login rate limiting",
  () => fixture(async f => {
    const revoked = await fetch(
      `${f.admin.origin}/api/host/oauth2/revoke`,
      {
        method: "POST",
    ...
    assert.equal((await f.request("admin", "/api/events")).status, 401);
    f.advance(60_001);
    for (let index = 0; index < 10; index++) assert.equal((await f.request("admin", "/api/host/login", "POST", { key: "invalid" }, "")).status, 401);
    assert.equal((await f.request("admin", "/api/host/login", "POST", { key: "invalid" }, "")).status, 429);
  })
);
test(
  "SQLite rejects a second writer and private files reject symlinks",
  () => fixture(async f => {
...
test(
  "Input validation, metadata allowlist, safe LAN opt-in and stable identifiers",
  async () => {
    const metadata = {
      id: "demo",
      name: "Demo",
    ...
    const sanitized = required(publicMetadata(JSON.stringify(metadata), "/repo/problems/challenges/demo/metadata.json"));
    for (const hidden of [
      "secret",
      "early",
      "SECRET",
    ...
    assert.throws(() => parseOptions(["--lan", "192.168.1.2"], "/tmp"));
    assert.throws(() => parseOptions(["--admin-port", "5175"], "/tmp"));
    assert.equal(parseOptions(["--lan", "192.168.1.2", "--unsafe-lan"], "/tmp").hostname, "192.168.1.2");
...
test(
  "Multiple teammates get independent one-use handoffs",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
    await f.begin(event.eventId);
...
test(
  "Lost and interrupted environments become retryable after restart",
  () => fixture(async f => {
    const event = await f.create();
    await f.deploy(event.eventId);
...
test(
  "Hosting build excludes unrelated problem code, templates and installed packs",
  async () => {
...
const c = import.meta.glob("../../../../.tenkacloud/pack-store/snapshots/**/metadata.json");`;
    const transformed = required(narrowCatalog(code, "/repo/apps/application-admin-console/src/data/problems.ts"));
    assert.ok(!transformed.includes("pack-store"));
    assert.ok(!transformed.includes("problems/*/*"));
    assert.ok(transformed.includes("import.meta.glob([])"));
    const plugins = required(narrowCatalog(
    ...
    assert.doesNotThrow(() => assertHostingModule("/repo/problems/challenges/sqli-demo/metadata.json"));
    assert.throws(() => assertHostingModule("/repo/problems/challenges/sqli-demo/local/app/server.mjs"));
    ...
    assert.throws(() => narrowCatalog("changed-glob", "/repo/apps/participant-portal/src/plugins/loader.ts"));
  }
);
...
async function main(): Promise<void> {
  ...
  const report = {
    ...
    scope: "Real HTTP and SQLite control-plane integration with a test-only exercise adapter, not production Docker/UI verification",
    results
  };
  if (process.env.HOST_TEST_REPORT) writeFileSync(process.env.HOST_TEST_REPORT, `${JSON.stringify(report, null, 2)}\n`);
