...
export class SurfaceGateways {
  ...
  private async create(job: Job): Promise<Gateway> {
    ...
    async function handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
      ...
      const token =
        cookies.find((part) => part.startsWith(`${cookieName}=`))?.slice(cookieName.length + 1) ??
        "";
      ...
      const allowed =
        (method === "GET" && ["/", "/healthz"].includes(url.pathname)) ||
        (method === "POST" && url.pathname === "/login");
      ...
      if (
        method === "POST" &&
        !/^(application\/json|application\/x-www-form-urlencoded)(?:;|$)/iu.test(type)
      )
        throw new HostError(415, "Unsupported challenge input.");
      ...
      if (!/^(text\/html|application\/json)(?:;|$)/iu.test(responseType))
