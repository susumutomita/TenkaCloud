...
| `landing/` | Landing-site generators: `generate-landing-docs.ts`, `generate-landing-locales.ts` (both support `--check`), `landing-seo.test.ts`, and `onboarding-videos/` (the onboarding drill video scripts + `render.ts` CLI that regenerates `landing/videos/onboarding/*.mp4`) |
