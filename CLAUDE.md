...
このスキルは、ジェネリックな「AI 風」デザインを避け、プロダクショングレードで独自性のあるフロントエンドインターフェースを作成するためのガイドラインである。
...
**CRITICAL**: 明確なコンセプト方向性を選び、精密に実行する。大胆なマキシマリズムも洗練されたミニマリズムも有効である。鍵は意図性であり、強度ではない。
...
- **カラーとテーマ**: 一貫した美学にコミットする。CSS 変数で一貫性を保つ。シャープなアクセントを持つドミナントカラーは、控えめで均等に分散されたパレットより効果的である。
- **モーション**: エフェクトとマイクロインタラクションにアニメーションを使用する。HTML には CSS のみのソリューションを優先する。React では利用可能な場合 Motion ライブラリを使用する。インパクトのある瞬間に集中する。散在するマイクロインタラクションより、スタッガードリビール（animation-delay）を伴う一つの洗練されたページロードの方が喜ばれる。驚きを与えるスクロールトリガーとホバー状態を使用する。
