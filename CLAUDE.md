...
| `make infra-coverage-check` | Infrastructure critical file coverage ratchet — fails only when AssumeRole/ExternalId, tenant-isolation, deploy-state-machine, scoring, delete-lifecycle, or auth-boundary file coverage drops past `scripts/quality/infra-critical-coverage-baseline.json` |
| `make infra-coverage-baseline` | Re-freeze the infra critical file coverage baseline (decreases must be justified in the PR body) |
