...
> **言い回し (voice) の正本は問題リポジトリ側の skill** ── `problems/.claude/skills/new-problem/SKILL.md` の "Voice" 規約に従う (個人名禁止＝役職のみ・前任者 blameless・AWS 架空企業名禁止・AI 臭さ禁止)。 メンテ重複を避けるため TenkaCloud 側では再定義しない。
